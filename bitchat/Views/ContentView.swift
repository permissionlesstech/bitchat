                        ...
                        Task {
                            do {
                                ...
                                guard let contentData = try? JSONSerialization.data(withJSONObject: fileContent),
                                    ...
                                    await MainActor.run {
                                        fileImportError = "Failed to encode file content"
                                    }
                                    return
                                }
                                
                                // Get identity for this geohash
                                let identity = try NostrIdentityBridge.deriveIdentity(forGeohash: geohashChannel.geohash)
                                ...
                                let event = try NostrProtocol.createEphemeralGeohashFileEvent(
                                    ...
                                    geohash: geohashChannel.geohash,
                                ...
                                let targetRelays = await GeoRelayDirectory.shared.closestRelays(
                                    toGeohash: geohashChannel.geohash,
                                    count: 3
                                )
                                
                                if targetRelays.isEmpty {
                                    await MainActor.run {
                                        fileImportError = "No relays available for location \(geohashChannel.geohash)"
                                    }
                                    return
                                }
                                
                                // Send to relays
                                await NostrRelayManager.shared.sendEvent(event, to: targetRelays)
                                
                            } catch {
                                await MainActor.run {
                                    fileImportError = "Failed to send file: \(error.localizedDescription)"
                                }
                            }
                        }
                    } else {
                        fileImportError = "Select a private chat or location channel first."
                    }
                } catch {
