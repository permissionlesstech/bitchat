            ...
            ZStack {
                            ...
                            .onEnded { value in
                                ...
                                withAnimation(.interactiveSpring()) {
                                    if showSidebar {
                                        ...
                                        if translation < -geometry.size.width / 4 {
                                            showSidebar = true
                                        }
                                    }
                                }
                ...
                if viewModel.selectedPrivateChatPeer != nil {
                    privateChatView
                        ...
                        .offset(x: (showPrivateChat ? 0 : geometry.size.width) + (dragOffset > 0 ? dragOffset : 0))
                        .animation(.interactiveSpring(), value: showPrivateChat)
                        .gesture(
                            DragGesture()
                                .updating($dragOffset) { value, state, _ in
                                    state = value.translation.width
                                ...
                                .onEnded { value in
                                    // Ensure the swipe is mostly horizontal to avoid conflicts with vertical scrolling
                                    guard abs(value.translation.width) > abs(value.translation.height) * 1.5 else { return }

                                    // Dismiss if swiped more than 1/3 of the screen width or with enough velocity
                                    if value.predictedEndTranslation.width > geometry.size.width / 3 {
                                        // The existing .onChange will handle animating the view away
                ...
                .offset(x: (showSidebar ? 0 : geometry.size.width) + dragOffset)
    ...
    private func messagesView(privatePeer: String?, isAtBottom: Binding<Bool>) -> some View {
        ScrollViewReader { proxy in
                            ...
                            if message.sender == "system" {
                                ...
                                VStack(alignment: .leading, spacing: 0) {
                                    HStack(alignment: .top, spacing: 0) {
                                        let isLong = (message.content.count > TransportConfig.uiLongMessageLengthThreshold || message.content.hasVeryLongToken(threshold: TransportConfig.uiVeryLongTokenThreshold))
                                    ...
                                    if (message.content.count > TransportConfig.uiLongMessageLengthThreshold || message.content.hasVeryLongToken(threshold: TransportConfig.uiVeryLongTokenThreshold)) {
                                        let isExpanded = expandedMessageIDs.contains(message.id)
            ...
            .onChange(of: viewModel.messages) { oldValue, newValue in
                if privatePeer == nil && !newValue.isEmpty {
                    ...
                    if now.timeIntervalSince(lastScrollTime) > TransportConfig.uiScrollThrottleSeconds {
                        ...
                        let target = newValue.suffix(count).last.map { "\(contextKey)|\($0.id)" }
                        DispatchQueue.main.async {
        ...
        VStack(spacing: 0) {
            ...
            if showCommandSuggestions && !commandSuggestions.isEmpty {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    let isGeoPublic: Bool = false
                    let isGeoDM: Bool = false
                ...
                // iOS keyboard autocomplete and capitalization enabled by default
                    .onChange(of: messageText) { newValue in
                        ...
                        if newValue.hasPrefix("/") && newValue.count >= 1 {
                            ...
                            commandSuggestions = Set(commandSuggestions).sorted()
        ...
        HStack(spacing: 0) {
            ...
            HStack(spacing: 10) {
                ...
                if viewModel.hasAnyUnreadMessages {
                    ...
                    .onChange(of: viewModel.unreadPrivateMessages, initial: true) {
            ...
            Button(action: {
                ...
                HStack(spacing: 6) {
                    ...
                    Text("\(privatePeerNick)")
                        ...
                        .foregroundStyle(textColor)                            // Dynamic encryption status icon (hide for geohash DMs)
