...
private struct BLEAuthenticatedPeerStateObservation: Equatable {
...
final class BLEService: NSObject {
    ...
    func authenticatedPeerTransportState(
        ...
        let observation = collectionsQueue.sync {
            () -> BLEAuthenticatedPeerStateObservation? in
            guard privateMediaSessionGenerations[normalizedPeerID] == generation,
                  let authenticated = authenticatedPeerStates[normalizedPeerID],
        ...
        guard let observation,
              noiseService.sessionGeneration(for: normalizedPeerID) == generation
        else {
            return nil
        }
    ...
    private func sendAnnounceNow(forceSend: Bool) {
        ...
        let (connectedPeerIDs, advertisedCapabilities, advertisedBridgeCell): ([Data], PeerCapabilities, String?) = collectionsQueue.sync {
            var capabilities = PeerCapabilities.localSupported.union(runtimeCapabilities)
            if doubleRatchetEnabled {
                capabilities.insert(.doubleRatchet)
            }
            return (
    ...
    private func handleAuthenticatedPeerState(
        ...
        guard let application = noiseService.withCurrentSessionGeneration(
            ...
            expected: generation,
            {
                ...
                let update = collectionsQueue.sync(flags: .barrier) {
                    () -> (
                        completions: [@MainActor (PrivateMediaSendPolicy) -> Void],
                        observationChanged: Bool
                    ) in
                    ...
                    let observation = BLEAuthenticatedPeerStateObservation(
                        fingerprint: fingerprint,
                        sessionGeneration: generation,
                        capabilities: state.capabilities
                    )
                    let observationChanged =
                        authenticatedPeerStates[normalizedPeerID] != observation
                    authenticatedPeerStates[normalizedPeerID] = observation
                    privateMediaProofTimeoutMarkers.removeValue(forKey: normalizedPeerID)
                    ...
                    return (
                        Array(pending.completions.values),
                        observationChanged
                    )
                }
                return (
