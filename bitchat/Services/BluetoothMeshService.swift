...
// MARK: - BluetoothMeshService

// A service that manages Bluetooth mesh networking, including encryption, peer discovery,
// message delivery, and peer status tracking across the Bitchat network.
//
// - Note: This class is marked `@unchecked Sendable` due to manual thread-safety control.
...
class BluetoothMeshService: NSObject, @unchecked Sendable {
    ...
    override init() {
        ...
        myPeerID = randomBytes.map { String(format: "%02x", $0) }.joined()
        ...
        bloomFilterResetTimer = Timer.scheduledTimer(
            withTimeInterval: 300.0,
            repeats: true
        ) { [weak self] _ in
            self?.messageQueue.async(flags: .barrier) {
                ...
                self.messageBloomFilter = OptimizedBloomFilter.adaptive(
                    for: networkSize
                )
                ...
                print(
                    "[BloomFilter] Reset with network size: \(networkSize), memory: \(self.messageBloomFilter.memorySizeBytes) bytes"
                )
            }
        }
    ...
    static let serviceUUID = CBUUID(
        string: "F47B5E2D-4A9E-4C5A-9B3F-8E1D2C3A4B5C"
    )
    static let characteristicUUID = CBUUID(
        string: "A1B2C3D4-E5F6-4A5B-8C9D-0E1F2A3B4C5D"
    )
    ...
    func getPeerPublicKey(_ peerID: String) -> Data? {
        encryptionService.getPeerIdentityKey(peerID)
    }
    ...
    func sendBroadcastAnnounce() {
        guard let vm = delegate as? ChatViewModel else {
            return
        }
        ...
        for baseDelay in [0.5, 1.0, 2.0] {
            let jitteredDelay = baseDelay + randomDelay()
    ...
    func sendMessage(
        ...
        messageQueue.async { [weak self] in
            ...
            let nickname = delegate as? ChatViewModel
            let senderNick = nickname?.nickname ?? myPeerID
            ...
            if let messageData = message.toBinaryPayload() {
                ...
                let signature: Data?
                do {
                    signature = try encryptionService.sign(messageData)
                } catch {
                ...
                let msgID =
                    "\(packet.timestamp)-\(myPeerID)-\(packet.payload.prefix(32).hashValue)"

                recentlySentMessagesLock.lock()
                let shouldSend = !recentlySentMessages.contains(msgID)
                if shouldSend {
                    recentlySentMessages.insert(msgID)
                }
                recentlySentMessagesLock.unlock()

                if shouldSend {
                    // Clean up old entries after 10 seconds
                    messageQueue.asyncAfter(deadline: .now() + 10.0) {
                        [weak self] in
                        guard let self else {
                            return
                        }
                        recentlySentMessagesLock.lock()
                        recentlySentMessages.remove(msgID)
                        recentlySentMessagesLock.unlock()
                    }
                    ...
                    let retryDelay = 0.3 + randomDelay()
    ...
    func sendPrivateMessage(
        _ content: String,
        to recipientPeerID: String,
        recipientNickname: String,
        messageID: String? = nil
    ) {
        // Defensive checks
        guard !content.isEmpty, !recipientPeerID.isEmpty,
            !recipientNickname.isEmpty
        else {
            return
        }
        ...
        messageQueue.async { [weak self] in
            ...
            if let messageData = message.toBinaryPayload() {
                ...
                let blockSize = MessagePadding.optimalBlockSize(
                    for: messageData.count
                )
                let paddedData = MessagePadding.pad(
                    messageData,
                    toSize: blockSize
                )
                ...
                do {
                    encryptedPayload = try encryptionService.encrypt(
                        paddedData,
                        for: recipientPeerID
                    )
                } catch {
                ...
                let signature: Data?
                do {
                    signature = try encryptionService.sign(encryptedPayload)
                } catch {
                ...
                let isRecipientOffline = !activePeers.contains(recipientPeerID)
                activePeersLock.unlock()

                if isRecipientOffline {
                    if let publicKeyData = encryptionService.getPeerIdentityKey(
                        recipientPeerID
                    ) {
                        let fingerprint = getPublicKeyFingerprint(publicKeyData)
                        if delegate?.isFavorite(fingerprint: fingerprint)
                            ?? false
                        {
                            // Recipient is offline favorite, cache the message
                            let messageID = "\(packet.timestamp)-\(myPeerID)"
                            cacheMessage(packet, messageID: messageID)
                        }
                    }
                }
    ...
    func announcePasswordProtectedChannel(
        _ channel: String,
    ...
    func sendChannelRetentionAnnouncement(_ channel: String, enabled: Bool) {
        messageQueue.async { [weak self] in
            ...
            let payload = "\(channel)|\(enabledFlag)|\(myPeerID)"
    ...
    func sendEncryptedChannelMessage(
        _ content: String,
        mentions: [String],
        channel: String,
    ...
    private var fragmentMetadata:
        [String: (originalType: UInt8, totalFragments: Int, timestamp: Date)] =
            [:]
    ...
    private var pendingMessages:
        [(message: BitchatPacket, destination: String?)] = []
    ...
    private var messageBloomFilter = OptimizedBloomFilter(
        expectedItems: 2000,
    ...
    private var estimatedNetworkSize: Int {
        max(activePeers.count, connectedPeripherals.count)
    }
    ...
    private func startAggregationTimer() {
        aggregationTimer?.invalidate()
        aggregationTimer = Timer.scheduledTimer(
            withTimeInterval: aggregationWindow,
            repeats: false
        ) { [weak self] _ in
    ...
    private func flushPendingMessages() {
        ...
        messageQueue.async { [weak self] in
            ...
            for (message, destination) in pendingMessages {
            ...
            for (destination, messages) in messagesByDestination {
                if messages.count == 1 {
                    ...
                    if destination == nil {
                        broadcastPacket(messages[0])
                    } else if let dest = destination,
                        let peripheral = connectedPeripherals[dest],
                        peripheral.state == .connected,
                        let characteristic = peripheralCharacteristics[
                            peripheral
                        ]
                    {
                    ...
                    for (index, message) in messages.enumerated() {
                        ...
                        { [weak self] in
                            if destination == nil {
                                ...
                                let peripheral = self?.connectedPeripherals[
                                    dest
                                ],
                                peripheral.state == .connected,
    ...
    private func scheduleScanDutyCycle() {
        guard scanDutyCycleTimer == nil else {
            return
        }
        ...
        scanDutyCycleTimer = Timer.scheduledTimer(
            withTimeInterval: activeScanDuration,
            repeats: true
        ) { [weak self] _ in
            guard let self else {
                return
            ...
            if isActivelyScanning {
                ...
                // Schedule resume
                DispatchQueue.main.asyncAfter(
                    deadline: .now() + scanPauseDuration
                ) { [weak self] in
                    guard let self else {
                        return
                    }
    ...
    private func setupPeripheral() {
        ...
        let service = CBMutableService(
            type: BluetoothMeshService.serviceUUID,
            primary: true
        )
    ...
    private func sendAnnouncementToPeer(_ peerID: String) {
        ...
        if let data = packet.toBinaryData() {
            ...
            if let peripheral = connectedPeripherals[peerID],
                peripheral.state == .connected,
                let characteristic = peripheral.services?.first(where: {
                    $0.uuid == BluetoothMeshService.serviceUUID
                })?.characteristics?.first(where: {
                    $0.uuid == BluetoothMeshService.characteristicUUID
                })
            {
    ...
    private func notifyPeerListUpdate(immediate: Bool = false) {
        if immediate {
            ...
            let connectedPeerIDs = getAllConnectedPeerIDs()
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                peerListUpdateTimer = Timer.scheduledTimer(
                    withTimeInterval: peerListUpdateDebounceInterval,
                    repeats: false
                ) { [weak self] _ in
                    guard let self else {
                        return
                    ...
                    delegate?.didUpdatePeerList(connectedPeerIDs)
                }
            }
        }
    }
    ...
    private func cleanupStalePeers() {
        ...
        for peerID in peersToRemove {
            ...
            processedKeyExchanges = processedKeyExchanges.filter {
                !$0.contains(peerID)
            }
        ...
        messageQueue.async(flags: .barrier) { [weak self] in
            ...
            if packet.type == MessageType.message.rawValue,
                ...
                let recipientPeerID = String(
                    data: recipientID.trimmingNullBytes(),
                    encoding: .utf8
                )
            {
            ...
            let storedMessage = StoredMessage(
                packet: packet,
                timestamp: Date(
                    timeIntervalSince1970: TimeInterval(packet.timestamp)
                        / 1000.0
            ...
            if isForFavorite {
                ...
                {
                    if favoriteMessageQueue[recipientPeerID] == nil {
                        favoriteMessageQueue[recipientPeerID] = []
                    }
                    favoriteMessageQueue[recipientPeerID]?.append(storedMessage)

                    // Limit favorite queue size
                    if let count = favoriteMessageQueue[recipientPeerID]?.count,
                        count > maxCachedMessagesForFavorites
                    {
                        favoriteMessageQueue[recipientPeerID]?.removeFirst()
                    }
                }
            } else {
    ...
    private func cleanupMessageCache() {
        ...
        messageCache.removeAll {
            !$0.isForFavorite && $0.timestamp < cutoffTime
        }
    ...
    private func sendCachedMessages(to peerID: String) {
        messageQueue.async { [weak self] in
            ...
            cachedMessagesSentToPeer.insert(peerID)
            ...
            if let favoriteMessages = favoriteMessageQueue[peerID] {
                // Filter out already delivered messages
                let undeliveredFavoriteMessages = favoriteMessages.filter {
                    !self.deliveredMessages.contains($0.messageID)
                }
                messagesToSend.append(contentsOf: undeliveredFavoriteMessages)
                // Clear the favorite queue after adding to send list
                favoriteMessageQueue[peerID] = nil
            }

            // Filter regular cached messages for this specific recipient
            let recipientMessages = messageCache.filter { storedMessage in
            ...
            let messageIDsToRemove = messagesToSend.map(\.messageID)
            deliveredMessages.formUnion(messageIDsToRemove)
            ...
            if !messageIDsToRemove.isEmpty {
                messageQueue.async(flags: .barrier) {
                    ...
                    if var favoriteQueue = self.favoriteMessageQueue[peerID] {
                        ...
                        self.favoriteMessageQueue[peerID] =
                            favoriteQueue.isEmpty ? nil : favoriteQueue
                    }
                }
            }
        }
        ...
        for (_, peripheral) in connectedPeripherals {
            if let characteristic = peripheralCharacteristics[peripheral] {
                ...
                if peripheral.state == .connected {
                    ...
                    let writeType: CBCharacteristicWriteType =
                        data.count > 512 ? .withResponse : .withoutResponse
                    ...
                    if let peerID = connectedPeripherals.first(where: {
                        $0.value == peripheral
                    })?.key {
                        connectedPeripherals.removeValue(forKey: peerID)
                        peripheralCharacteristics.removeValue(
                            forKey: peripheral
                        )
                    }
                }
    ...
    private func handleReceivedPacket(
        ...
        messageQueue.async(flags: .barrier) { [weak self] in
            ...
            if let senderID = String(
                data: packet.senderID.trimmingNullBytes(),
                encoding: .utf8
            ),
            ...
            let messageID =
                    ...
                    || packet.type == MessageType.fragmentEnd.rawValue
                {
                    ...
                    "\(packet.timestamp)-\(String(data: packet.senderID.trimmingNullBytes(), encoding: .utf8) ?? "")-\(packet.type)-\(packet.payload.hashValue)"
                } else {
                    // Include payload hash for absolute uniqueness (handles same-second messages)
                    "\(packet.timestamp)-\(String(data: packet.senderID.trimmingNullBytes(), encoding: .utf8) ?? "")-\(packet.payload.prefix(64).hashValue)"
                }
            ...
            if messageBloomFilter.contains(messageID) {
                ...
                if processedMessages.contains(messageID) {
                    ...
                    print(
                        "[BloomFilter] False positive detected for message: \(messageID)"
                    )
                }
            }
            ...
            if messageBloomFilter.insertCount % 100 == 0 {
                ...
                print(
                    "[BloomFilter] Items: \(messageBloomFilter.insertCount), Est. FP rate: \(String(format: "%.3f%%", fpRate * 100))"
                )
            }
                ...
                if let recipientID = packet.recipientID {
                    if recipientID == SpecialRecipients.broadcast {
                        ...
                        if let signature = packet.signature {
                            do {
                                let isValid = try encryptionService.verify(
                                    signature,
                                    for: packet.payload,
                                    from: senderID
                                )
                        ...
                        if let message = BitchatMessage.fromBinaryPayload(
                            packet.payload
                        ) {
                                ...
                                let encryptedData = message.encryptedContent
                            {
                                ...
                                if let decryptedContent = delegate?
                            ...
                            if let peerID = String(
                                data: packet.senderID.trimmingNullBytes(),
                                encoding: .utf8
                            ) {
                                lastMessageFromPeer[peerID] = Date()
                            }
                            ...
                            let viewModel = delegate as? ChatViewModel
                            let myNickname = viewModel?.nickname ?? myPeerID
                            if message.channel != nil,
                                let mentions = message.mentions,
                                mentions.contains(myNickname)
                                    || self.activePeers.count < 10
                            {
                                if let ack = DeliveryTracker.shared.generateAck(
                                    for: messageWithPeerID,
                                    myPeerID: myPeerID,
                                    myNickname: myNickname,
                                    hopCount: UInt8(maxTTL - packet.ttl)
                                ) {
                                    sendDeliveryAck(ack, to: senderID)
                                }
                            }
                        }
                    ...
                    } else if let recipientIDString = String(
                        data: recipientID.trimmingNullBytes(),
                        encoding: .utf8
                    ),
                        recipientIDString == myPeerID
                    {
                        ...
                        do {
                            let decryptedPadded = try encryptionService.decrypt(
                                packet.payload,
                                from: senderID
                            )

                            // Remove padding
                            decryptedPayload = MessagePadding.unpad(
                                decryptedPadded
                            )
                        } catch {
                        ...
                        if let recipientIDString = String(
                            data: recipientID.trimmingNullBytes(),
                            encoding: .utf8
                        ),
                            let publicKeyData =
                                encryptionService.getPeerIdentityKey(
                                    recipientIDString
                                )
                        {
                            let fingerprint = getPublicKeyFingerprint(
                                publicKeyData
                            )
                            // Only cache if recipient is a favorite AND is currently offline
                            if delegate?.isFavorite(fingerprint: fingerprint)
                                ?? false,
                                !activePeers.contains(recipientIDString)
                            {
                                cacheMessage(relayPacket, messageID: messageID)
                            }
                        }

                        // Private messages are important - use higher relay probability
                        let relayProb = min(
                            adaptiveRelayProbability + 0.15,
                            1.0
                        )  // Boost by 15%
            ...
            case .keyExchange:
                ...
                if let senderID = String(
                    ...
                    if !packet.payload.isEmpty {
                        ...
                        let exchangeKey =
                            "\(senderID)-\(publicKeyData.hexEncodedString().prefix(16))"
                        ...
                        do {
                            try encryptionService.addPeerPublicKey(
                                senderID,
                        ...
                        if let viewModel = delegate as? ChatViewModel,
                            let identityKeyData =
                                encryptionService.getPeerIdentityKey(senderID)
                        {
                            viewModel.registerPeerPublicKey(
                                peerID: senderID,
                                publicKeyData: identityKeyData
                            )
                        }

                        // If we have RSSI from discovery, apply it to this peer
                        if let peripheral,
                            let tempRSSI = peripheralRSSI[
                        ...
                        sendAnnouncementToPeer(senderID)
                    ...
                    )
                {
                    ...
                    for (existingPeerID, existingNickname) in peerNicknames {
                        if existingNickname == nickname,
                            existingPeerID != senderID
                        {
                            // Check if this peer was seen very recently (within 10 seconds)
                            let wasRecentlySeen =
                                peerLastSeenTimestamps[existingPeerID].map {
                                    Date().timeIntervalSince($0) < 10.0
                                } ?? false
                            if !wasRecentlySeen {
                            ...
                            DispatchQueue.main.async {
                                ...
                                DispatchQueue.main.asyncAfter(
                                        ...
                                        )
                                    {
                                        ...
                                        if delegate?.isFavorite(
                                            fingerprint: fingerprint
                                        ) ?? false {
                                            NotificationService.shared
                                                .sendFavoriteOnlineNotification(
                                                    nickname: nickname
                                                )

                                            // Send any cached messages for this favorite
                                            sendCachedMessages(to: senderID)
                                        }
                                    }
                                }
                            }
            ...
            case .leave:
                if let senderID = String(
                        ...
                        channel.hasPrefix("#")
                    {
                        ...
                        DispatchQueue.main.async {
                            self.delegate?.didReceiveChannelLeave(
                                channel,
                                from: senderID
                            )
                        }
            ...
            case .channelAnnounce:
                if let payloadStr = String(
                    data: packet.payload,
                    encoding: .utf8
                ) {
                    // Parse payload: channel|isProtected|creatorID|keyCommitment
                    let components = payloadStr.split(separator: "|").map(
                        String.init
                    )
                    if components.count >= 3 {
                        ...
                        DispatchQueue.main.async {
                            self.delegate?
                                .didReceivePasswordProtectedChannelAnnouncement(
                                    channel,
            ...
            case .deliveryAck:
                ...
                if let recipientIDData = packet.recipientID,
                    let recipientID = String(
                        data: recipientIDData.trimmingNullBytes(),
                        encoding: .utf8
                    ),
            ...
            case .channelRetention:
                if let payloadStr = String(
                    ...
                    if components.count >= 3 {
                        ...
                        DispatchQueue.main.async {
                            self.delegate?
                                .didReceiveChannelRetentionAnnouncement(
                                    channel,
                                    enabled: enabled,
                                    creatorID: creatorID
                                )
                    ...
                    recipientID == myPeerID
                {
                    ...
                    if let senderID = String(
                        ...
                        do {
                            ...
                            if let receipt = ReadReceipt.decode(
                                from: decryptedData
                            ) {
                                // Process the read receipt
                                DispatchQueue.main.async {
                                    self.delegate?.didReceiveReadReceipt(
                                        receipt
                                    )
                                }
                            }
    ...
    private func sendFragmentedPacket(_ packet: BitchatPacket) {
        ...
        let fragments = stride(from: 0, to: fullData.count, by: maxFragmentSize)
            .map { offset in
        ...
        for (index, fragmentData) in fragments.enumerated() {
            ...
            messageQueue.asyncAfter(deadline: .now() + totalDelay) {
                [weak self] in
                self?.broadcastPacket(fragmentPacket)
            }
        }

        _ = Double(fragments.count - 1) * delayBetweenFragments
    }
    ...
    private func handleFragment(_ packet: BitchatPacket, from peerID: String) {
        ...
        let index =
            Int(payloadArray[offset]) << 8 | Int(payloadArray[offset + 1])
        offset += 2
    ...
    func centralManager(
        ...
        if error != nil {
            ...
            if attempts >= maxConnectionAttempts {
                ...
                let backoffDuration =
                    baseBackoffInterval * pow(2.0, Double(attempts))
                connectionBackoff[peripheralID] =
                    Date().timeIntervalSince1970 + backoffDuration
            }
        } else {
        ...
        if let peerID = foundPeerID {
            ...
            if let nickname, nickname != peerID {
                DispatchQueue.main.async {
                    self.delegate?.didDisconnectFromPeer(nickname)
                }
            }
...
extension BluetoothMeshService: CBPeripheralDelegate {
    func peripheral(
        ...
        for service in services {
            peripheral.discoverCharacteristics(
                [BluetoothMeshService.characteristicUUID],
                for: service
            )
        }
    }
    ...
    func peripheral(
        ...
        let _ =
            connectedPeripherals.first(where: { $0.value == peripheral })?.key
            ?? "unknown"
        let packetSenderID =
            String(data: packet.senderID.trimmingNullBytes(), encoding: .utf8)
            ?? "unknown"
...
extension BluetoothMeshService: CBPeripheralManagerDelegate {
                ...
                let packet = BitchatPacket.from(data)
            {
                ...
                if peerID != "unknown", peerID != myPeerID {
                    ...
                    if packet.type == MessageType.keyExchange.rawValue {
                        let publicKeyData =
                            encryptionService.getCombinedPublicKeyData()
    ...
    private func disconnectLeastImportantPeripherals(keepCount: Int) {
        ...
        let sortedPeripherals = connectedPeripherals.values
            .sorted { peer1, peer2 in
                ...
                let peer1Activity =
                    lastMessageFromPeer[peer1.identifier.uuidString]
                    ?? Date.distantPast
                let peer2Activity =
                    lastMessageFromPeer[peer2.identifier.uuidString]
                    ?? Date.distantPast
    ...
    private func scheduleAdvertisingCycle(interval: TimeInterval) {
        ...
        advertisingTimer = Timer.scheduledTimer(
    ...
    private func advertiseBurst() {
        guard
            batteryOptimizer.currentPowerMode != .ultraLowPower
                || !batteryOptimizer.isInBackground
        else {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
            if self?.batteryOptimizer.currentPowerMode.advertisingInterval ?? 0
                > 0
            {
    ...
    private func randomDelay() -> TimeInterval {
        ...
        TimeInterval.random(in: minMessageDelay...maxMessageDelay)
    }
    ...
    private func scheduleCoverTraffic() {
        ...
        coverTrafficTimer?.invalidate()
        coverTrafficTimer = Timer.scheduledTimer(
            withTimeInterval: interval,
            repeats: false
        ) { [weak self] _ in
    ...
    private func sendDummyMessage() {
        ...
        guard let randomPeer = peers.randomElement() else {
            return
        }
