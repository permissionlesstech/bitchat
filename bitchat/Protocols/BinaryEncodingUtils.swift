...
extension Data {
    ...
    func crc32() -> UInt32 {
        return self.withUnsafeBytes { bytes in
            ...
            return crc32_calculate(buffer.baseAddress, buffer.count)
        }
    }
}

// MARK: - BigEndian Extensions
extension UInt32 {
...
extension Data {
    ...
    mutating func appendTimeInterval(_ timeInterval: TimeInterval) {
        // Store as 64-bit double precision
    ...
    func readTimeInterval(at offset: inout Int) -> TimeInterval? {
        let size = MemoryLayout<TimeInterval>.size
        guard offset + size <= self.count else { return nil }
        
        let data = self[offset..<offset + size]
        offset += size
        
        return data.withUnsafeBytes { bytes in
            bytes.load(as: TimeInterval.self)
        }
    }
}
...
private func crc32_calculate(_ data: UnsafePointer<UInt8>?, _ length: Int) -> UInt32 {
