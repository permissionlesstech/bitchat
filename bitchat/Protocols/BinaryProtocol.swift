...
struct BinaryProtocol {
    ...
    static func encode(_ packet: BitchatPacket, padding: Bool = true) -> Data? {
        ...
        // header(13/15) + sender(8) + opt recipient(8) + opt originalSize(2) + payload + opt signature(64) + up to 255 pad
        let estimatedPayload = payload.count + (isCompressed ? 2 : 0)
        let estimatedHeader = (packet.version >= 2 ? headerSizeV2 : headerSizeV1)
        let estimated = estimatedHeader + senderIDSize + (packet.recipientID == nil ? 0 : recipientIDSize) + estimatedPayload + (packet.signature == nil ? 0 : signatureSize) + 255
        ...
        // Payload length (v1=2 bytes, v2+=4 bytes) - includes original size if compressed
        let payloadDataSize = payload.count + (isCompressed ? 2 : 0)
        if packet.version >= 2 {
            let len32 = UInt32(payloadDataSize)
            data.append(UInt8((len32 >> 24) & 0xFF))
            data.append(UInt8((len32 >> 16) & 0xFF))
            data.append(UInt8((len32 >> 8) & 0xFF))
            data.append(UInt8(len32 & 0xFF))
        } else {
    ...
    private static func decodeCore(_ raw: Data) -> BitchatPacket? {
        // Minimum size: header(v1) + senderID
        ...
        return raw.withUnsafeBytes { (buf: UnsafeRawBufferPointer) -> BitchatPacket? in
            ...
            // Payload length (v1=2 bytes, v2+=4 bytes)
            let payloadLen: Int
            if version >= 2 {
                guard let len32 = read32() else { return nil }
                payloadLen = Int(len32)
            } else {
                guard let len16 = read16() else { return nil }
                payloadLen = Int(len16)
            }
            ...
            if isCompressed {
                ...
                guard let p = readData(payloadLen) else { return nil }
                payload = p
            }
