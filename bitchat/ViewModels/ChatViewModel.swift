...
class ChatViewModel: ObservableObject, BitchatDelegate {
    ...
    private lazy var commandProcessor = CommandProcessor(viewModel: self)
    private lazy var privateChatManager = PrivateChatManager(viewModel: self)
    ...
    func sendMessage(_ content: String) {
        ...
        if content.hasPrefix("/") {
            Task { @MainActor in
                commandProcessor.handle(content)
            }
            return
        }
    ...
    func startPrivateChat(with peerID: String) {
        privateChatManager.startPrivateChat(with: peerID)
    }
    ...
    func markPrivateMessagesAsRead(from peerID: String) {
        privateChatManager.markPrivateMessagesAsRead(from: peerID)
    }
    
    func getPrivateChatMessages(for peerID: String) -> [BitchatMessage] {
        privateChatManager.getPrivateChatMessages(for: peerID)
    }
    ...
    private func trimPrivateChatMessagesIfNeeded(for peerID: String) {
        privateChatManager.trimPrivateChatMessagesIfNeeded(for: peerID)
    }
