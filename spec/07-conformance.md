...
- [ ] An `authenticatedPeerState` decoder MUST skip an unrecognized TLV type whose `length` lies within the payload, MUST reject the payload on any TLV whose `length` overruns it, and MUST reject the whole payload — not skip the entry — on a malformed TLV of a recognized type ([§5](04-payloads.md#5-peer-state-and-capabilities)).
- [ ] `PeerCapabilities` is little-endian and minimal-byte-length (trailing zero bytes stripped) and an encoder MUST NOT emit more than 8 bytes; a standalone `announce` decoder MUST accept any length, keeping the low 64 bits, while an `authenticatedPeerState` decoder MUST reject an empty, over-8-byte, or non-minimal field ([§5.1](04-payloads.md#51-peercapabilities-bitfield)).
...
The following capability bits, concepts, and structural limits are used or implied normatively elsewhere in this specification but have no defined wire mechanism of their own, or no defined remedy. They are not checklist items — there is nothing to check yet — and are noted here so an implementer does not mistake the absence of a checklist entry for the absence of the concept.
...
- **`authenticatedPeerState` can gain a field but cannot widen one.** [§5](04-payloads.md#5-peer-state-and-capabilities) of Payloads makes adding a TLV type forward-compatible — a decoder skips what it does not recognize — but offers no path for growing a field it already defines. A revision assigning `PeerCapabilities` bit 64 would need a ninth byte, which the same section requires a decoder to reject; and because rejection discards the payload whole rather than the single field, the peer's `signingPublicKey` and every other TLV it authenticated inside the session go with it. Nothing in this version can trigger it — [§5.1](04-payloads.md#51-peercapabilities-bitfield) reserves bits only through 63 — so this is a limit on how this payload may be revised, not a defect in it. Whether a future revision widens the field by degrading per-TLV instead of per-payload, or by assigning a second capabilities type, is left open.
