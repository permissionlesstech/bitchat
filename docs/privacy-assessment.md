...
- Each transport keeps its own Arti directory, so a route that has never run starts with no consensus and has to fetch one through the transport itself, which over Snowflake did not complete. A pluggable route is therefore seeded once from the directory the direct route already holds. What is copied is the public consensus, authority certificates, and microdescriptors every Tor client fetches identically, and Arti verifies the authority signatures on load, so the copy discloses nothing about the device and cannot be used to steer it. The copy is one-directional and never fills the direct route from a bridged one, keeping descriptors fetched through a bridge inside that bridge's own directory. Copying rather than sharing a single directory also keeps the routes off one lock, so a failure on one does not take the others with it. A bridged route consuming a directory originally fetched over the direct route is the residual: Arti's validity window bounds a network adversary on that path to replaying a still-fresh consensus rather than forging or staling one.
...
- Arti's directory tree is excluded from device backups, so guard state and any bridge descriptors cached beside it do not leave the device that way. It is deliberately given no file-protection class: Tor reads that tree while the device is locked, and anything stricter than the platform default would stop the app working in the background.
...
- **Cached bridge lines are not sealed.** A bridged route's Arti directory stores each bridge line in cleartext, as the primary key of the descriptor cache keyed by it. The Keychain copy is device-only and panic wipe removes the whole tree, but until a wipe those lines rely on the platform default protection class, readable once the device has been unlocked since boot. Possessing a bridge line is itself evidence of circumvention intent, so this is weaker than the material deserves.
